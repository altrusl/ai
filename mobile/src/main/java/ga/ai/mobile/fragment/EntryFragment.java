...
import ga.ai.mobile.activity.BaseActivity;
...
import ga.ai.mobile.view.EntryView;
...
public class EntryFragment extends SwipeRefreshFragment implements BaseActivity.OnFullScreenListener, LoaderManager.LoaderCallbacks<Cursor>, EntryView.EntryViewManager {
    ...
    private int mTitlePos = -1, mDatePos, mMobilizedHtmlPos, mAbstractPos, mLinkPos, mIsFavoritePos, mIsReadPos, mEnclosurePos, mAuthorPos, mCommentsPos, mFeedNamePos, mFeedUrlPos, mFeedIconPos;
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        if (mBaseUri != null && cursor != null) { // can be null if we do a setData(null) before
            ...
            if (mTitlePos == -1) {
                ...
                mCommentsPos = cursor.getColumnIndex(EntryColumns.COMMENTS);
...
//                    String comments = newCursor.getString(mCommentsPos);
//
//                    if (comments != null && !comments.isEmpty()) {
//                        contentText += "<br/><a href=\"http://altai-info.ga/mobile-app/feeds/feed-comments.php?url=" + comments + "&src=" + newCursor.getString(mFeedUrlPos) + "\"'>Посмотреть комментарии читателей</a>";
//                    }
//                    if (link.indexOf("-nga") > 0) {
//                        String num = link.substring(link.length() - 5);
//                        contentText += "<br/><a href=\"http://altai-info.ga/mobile-app/feeds/feed-comments.php?url=" + "http://www.gorno-altaisk.info/news/" + num + "/feed" + "&src=" + newCursor.getString(mFeedUrlPos) + "\"'>Посмотреть комментарии читателей</a>";
//                    }

//                    contentText = contentText.replaceAll("listock.ru/images", "listock-tunnel.ga/images");

                    contentText = "<script type=\"text/javascript\" src=\"file:///android_asset/ai-custom.js\"></script>\n" +
                            "<link rel=\"stylesheet\" href=\"file:///android_asset/ai-custom.css\" type=\"text/css\" />\n" + contentText;
//                    Log.d("------", contentText);
