...
import ga.ai.mobile.adapter.EntriesCursorAdapter;
    ...
    private final LoaderManager.LoaderCallbacks<Cursor> mEntriesNumberLoader = new LoaderManager.LoaderCallbacks<Cursor>() {
        ...
        public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
            ...
            if(mNewEntriesNumber!=0 && mListView != null) {
                try {
                    menu.findItem(R.id.menu_show_new_entries).getIcon().setColorFilter(
                            ContextCompat.getColor(mListView.getContext(), PrefUtils.getBoolean(PrefUtils.LIGHT_THEME, true) ? R.color.light_accent_color : R.color.dark_accent_color),
                            PorterDuff.Mode.MULTIPLY);
                } catch (NullPointerException e) {
                    // невыясненная ошибка иногда
                    e.printStackTrace();
                }
            }
...
//        menu.setGroupVisible(R.id.entry_list_group, isMenuVisible);
            ...
            if (cursor.moveToFirst()) {
                do {
                    if (cursor.isNull(mobilizedPos)) {
                        ...
                        //not mobilized, yet
//                        entries[i++] = (Long.valueOf(cursor.getChildListPosition()));
                    }
                } while (cursor.moveToNext());
            }
            if(i > 0) {
